...
const TerminatedEmployees = () => {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const res = await API.get("/users/terminated");
            ...
            <div style={{ fontFamily: "'DM Sans', sans-serif", paddingBottom: 40 }}>
                <div style={{ marginBottom: 24 }}>
                    ...
                    <p style={{ color: "var(--text-2)", fontSize: ".85rem", marginTop: 4 }}>
                        All terminated and removed employees — full history preserved
                    </p>
                </div>
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: ".83rem" }}>
                            <thead>
                                <tr style={{ background: "var(--surface-3)" }}>
                                    ...
                                    <th style={{ padding: "12px 16px", textAlign: "left", fontWeight: 700, fontSize: ".72rem", textTransform: "uppercase", letterSpacing: ".5px", color: "var(--text-2)" }}>Status</th>
                                    <th style={{ padding: "12px 16px", textAlign: "left", fontWeight: 700, fontSize: ".72rem", textTransform: "uppercase", letterSpacing: ".5px", color: "var(--text-2)" }}>Terminated On</th>
                            ...
                            <tbody>
                                {filtered.map(emp => (
                                    <tr key={emp._id} style={{ borderTop: "1px solid var(--border)" }}>
                                        ...
                                        <td style={{ padding: "12px 16px" }}>
                                            <span style={{
                                                background: emp.isDeleted ? "#fee2e2" : "#fef3c7",
                                                color: emp.isDeleted ? "#991b1b" : "#92400e",
                                                padding: "2px 8px", borderRadius: 4, fontSize: ".72rem", fontWeight: 700
                                            }}>
                                                {emp.isDeleted ? "Deleted" : "Terminated"}
                                            </span>
                                        </td>
                                        <td style={{ padding: "12px 16px", color: "var(--text-2)", fontSize: ".8rem" }}>
                                            {emp.deletedAt
                                                ? new Date(emp.deletedAt).toLocaleDateString("en-IN")
                                                : emp.updatedAt
