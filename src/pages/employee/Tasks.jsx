...
.tk-header h1 { font-size: 1.6rem; font-weight: 700; letter-spacing: -.5px; color: #111318; }
.tk-header p  { font-size: .825rem; color: #818898; margin-top: 2px; }
...
.tk-tab:hover:not(.active) { background: rgba(255,255,255,.6); color: #374151; }
.tk-tab.active {
    background: #fff;
    color: #4F46E5;
...
.tk-search:focus { border-color: #6366F1; }
...
.tk-add-btn:hover { background: #4F46E5; }
...
.badge-overdue    { background: #FFF1F2; color: #BE123C; }

.tk-card.overdue { border-color: #FECACA; }
...
.tk-action-btn:hover { background: #F3F4F6; border-color: #D1D5DB; }
.tk-action-btn.danger { color: #DC2626; }
.tk-action-btn.danger:hover { background: #FFF1F2; border-color: #FECACA; }
.tk-action-btn.primary { color: #4F46E5; }
.tk-action-btn.primary:hover { background: #EEF2FF; border-color: #C7D2FE; }
...
.tk-dept-row.header-row:hover { background: #FAFBFC; }
.tk-dept-name { font-weight: 600; color: #111318; }
.tk-dept-num  { text-align: center; }
...
.tk-close:hover { background: #E5E7EB; }
...
.tk-status-btn.active-pending    { background: #FFF7ED; color: #C2410C; border-color: #FED7AA; }
.tk-status-btn.active-inprogress { background: #EFF6FF; color: #1D4ED8; border-color: #BFDBFE; }
.tk-status-btn.active-done       { background: #F0FDF4; color: #15803D; border-color: #BBF7D0; }
...
.tk-save-btn:hover:not(:disabled) { background: #4F46E5; }
...
.tk-divider { border: none; border-top: 1px solid #F3F4F6; margin: 16px 0; }

/* confirm modal */
.tk-confirm-modal { max-width: 380px; }
.tk-confirm-text { font-size: .9rem; color: #374151; line-height: 1.6; margin-bottom: 20px; }
...
.tk-cancel-btn:hover { background: #E5E7EB; }
.tk-danger-btn {
    flex: 1;
    padding: 10px;
    background: #EF4444;
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px 20px", marginBottom: 18 }}>
                    <div>
                        ...
                        <div className="tk-assignee" style={{ marginTop: 4 }}>
                            <div className="tk-avatar">{initials(task.assignedBy?.name)}</div>
                            <span style={{ color: "#374151", fontWeight: 500 }}>{task.assignedBy?.name || "—"}</span>
                        </div>
                    </div>
                    <div>
                ...
                {task.workReport && task.workReport !== report && (
                    <div className="tk-modal-section">
                        ...
                        <div className="tk-modal-value" style={{
                            ...
                            borderRadius: 8, fontSize: ".82rem", whiteSpace: "pre-wrap",
                        }}>{task.workReport}</div>
                    </div>
                )}
            ...
            <div className="tk-modal" onClick={(e) => e.stopPropagation()}>
                ...
                {error && (
                    <div style={{
                        ...
                        padding: "10px 12px", fontSize: ".83rem", color: "#BE123C", marginBottom: 14,
                    }}>
                        {error}
                    </div>
                )}
...
const TaskCard = ({ task, canManage, onView, onEdit, onDelete }) => {
    ...
    return (
        ...
        >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {canManage ? (
                    <div className="tk-assignee">
                        <div className="tk-avatar" style={{ background: "#0EA5E9" }}>
                    ...
                    <div className="tk-assignee">
                        <div className="tk-avatar" style={{ background: "#818CF8" }}>
            ...
            {/* Action buttons only for TL/HR views — stop propagation so card click doesn't fire */}
            {canManage && (
...
const DeptOverviewTable = ({ deptBreakdown, onSelectDept }) => (
    <div className="tk-dept-table">
        ...
        {deptBreakdown.length === 0 && (
            <div style={{ padding: "24px 20px", textAlign: "center", color: "#9CA3AF", fontSize: ".83rem" }}>
                No data yet
            </div>
        )}
        {deptBreakdown.map((d) => (
            ...
            >
                ...
                <div className="tk-dept-num" style={{ color: "#C2410C" }}>{d.pending}</div>
                <div className="tk-dept-num" style={{ color: "#1D4ED8" }}>{d.inProgress}</div>
                <div className="tk-dept-num" style={{ color: "#15803D" }}>{d.done}</div>
            </div>
        ))}
    </div>
);
...
const Tasks = () => {
    ...
    // tab: "my" | "department" (TL) | "all" (HR/Manager)
    ...
    const [members, setMembers] = useState([]); // dept members for TL / all users for HR
    const [departments, setDepartments] = useState([]); // for HR filter

    // Filters
    const [statusFilter, setStatusFilter] = useState("all");
    const [priorityFilter, setPriorityFilter] = useState("all");
    const [deptFilter, setDeptFilter] = useState("all"); // HR only
    ...
    /* ── Fetch tasks based on current tab / filters ── */
    const fetchTasks = useCallback(async () => {
        try {
            ...
            if (tab === "my") {
                ...
                // HR / Manager — all tasks, optional dept filter
    ...
    /* ── Fetch members for TL / HR dropdown ── */
    useEffect(() => {
        const load = async () => {
            try {
                if (isTL) {
                    ...
                    // HR can assign to anyone — fetch all employees
                    const res = await API.get("/users?role=employee"); // adjust endpoint as needed
    ...
    useEffect(() => {
        ...
        else if (isTL || isHR) fetchStats(); // also fetch dept stats for header
            ...
            <DashboardLayout>
                <div className="tk-root">
                    ...
                    <div className="tk-header">
                        ...
                        {/* Add Task button for TL/HR on managed tabs */}
                        {currentTabIsManaged && (
                    ...
                    {/* Tabs — show only when role has multiple views */}
                    {tabs.length > 1 && (
                    ...
                    {/* Department overview table (HR, all-tasks tab, no filters) */}
                    {showDeptOverview && (
                        <DeptOverviewTable
                        ...
                        {!loading && filtered.length === 0 && (
                            <div className="tk-empty">
                                <div className="tk-empty-icon">
                                    <Icon d={icons.task} size={24} color="#9CA3AF" />
                                </div>
                                <p style={{ fontWeight: 600, color: "#374151", marginBottom: 6 }}>No tasks found</p>
                ...
                {/* TL/HR viewing a task in dept/all tab (read-only details) */}
                {viewTask && currentTabIsManaged && (
                        ...
                        <div className="tk-modal" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px 20px", marginBottom: 18 }}>
                                <div>
                                    ...
                                    <div className="tk-assignee" style={{ marginTop: 4 }}>
                                        <div className="tk-avatar" style={{ background: "#0EA5E9" }}>{initials(viewTask.assignedTo?.name)}</div>
                                        <span style={{ color: "#374151", fontWeight: 500 }}>{viewTask.assignedTo?.name || "—"}</span>
                                    </div>
                                </div>
                                <div>
                                    <div className="tk-modal-label">Assigned by</div>
                                    <div className="tk-assignee" style={{ marginTop: 4 }}>
                                        <div className="tk-avatar">{initials(viewTask.assignedBy?.name)}</div>
                                        <span style={{ color: "#374151", fontWeight: 500 }}>{viewTask.assignedBy?.name || "—"}</span>
                                    </div>
                                </div>
                                <div>
                                    <div className="tk-modal-label">Due date</div>
                            ...
                            {viewTask.workReport && (
                                <>
                                    ...
                                    <div className="tk-modal-section">
                                        ...
                                        <div className="tk-modal-value" style={{
                                            background: "#F9FAFB", padding: "10px 12px",
                                            borderRadius: 8, fontSize: ".85rem", whiteSpace: "pre-wrap",
